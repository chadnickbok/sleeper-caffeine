  ...
  async get<T>(
    ...
    if (!path.startsWith("/") || path.startsWith("//")) {
      throw new SleeperMcpError(
        "INVALID_INPUT",
        "Sleeper API paths must be absolute relative paths.",
      );
    }
    ...
    for (let attempt = 0; attempt <= this.#maxRetries; attempt += 1) {
      ...
      try {
        ...
        throw new SleeperMcpError(
          "SLEEPER_UNAVAILABLE",
          `Sleeper request failed for ${path}.`,
          {
            cause: error,
          },
        );
  ...
  #httpError(status: number, path: string): SleeperMcpError {
    if (status === 404) {
      return new SleeperMcpError(
        "SLEEPER_NOT_FOUND",
        `Sleeper resource was not found for ${path}.`,
      );
    }
    if (status === 429) {
      return new SleeperMcpError(
        "SLEEPER_RATE_LIMITED",
        "Sleeper rate-limited the request.",
      );
    }
    return new SleeperMcpError(
      "SLEEPER_UNAVAILABLE",
      `Sleeper returned HTTP ${status} for ${path}.`,
      {
        details: { status },
      },
    );
