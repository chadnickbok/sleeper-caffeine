...
export async function getTeamSnapshot(
  ...
  const [league, users, rosters, matchups, tradedPicks, directory] =
    await Promise.all([
  ...
  const teamMatchup = matchups.find(
    (matchup) => matchup.roster_id === resolved.roster.roster_id,
  );
  const opponentMatchup =
    teamMatchup?.matchup_id === undefined || teamMatchup.matchup_id === null
      ...
      : matchups.find(
          (matchup) =>
            matchup.matchup_id === teamMatchup.matchup_id &&
            matchup.roster_id !== resolved.roster.roster_id,
        );
  const opponentRoster =
    opponentMatchup === undefined
      ? undefined
      : rosters.find(
          (roster) => roster.roster_id === opponentMatchup.roster_id,
        );
  ...
  return {
    ...
    data: {
      ...
      user: {
        ...
        username:
          resolved.leagueUser.username ?? resolved.user.username ?? null,
        display_name:
          resolved.leagueUser.display_name ??
