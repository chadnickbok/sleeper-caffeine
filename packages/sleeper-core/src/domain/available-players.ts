...
export async function getAvailablePlayers(
  ...
  const trendingCounts = new Map(
    trending.map((entry) => [entry.player_id, entry.count]),
  );
  const positions = new Set(
    (input.positions ?? []).map((position) => position.toUpperCase()),
  );
  ...
  return {
    ...
    data: {
      ...
      definition:
        "Players absent from every current league roster; this does not assert waiver clearance or lineup eligibility.",
      filters: {
        positions: [...positions],
...
function playerMatchesPositions(
  player: PlayerSummary,
  positions: ReadonlySet<string>,
): boolean {
  return (
    (player.position !== null &&
      positions.has(player.position.toUpperCase())) ||
    player.fantasy_positions.some((position) =>
      positions.has(position.toUpperCase()),
    )
  );
}

function sortPlayers(
  sort: AvailablePlayersInput["sort"],
): (a: AvailablePlayer, b: AvailablePlayer) => number {
  if (sort === "name") {
  ...
  return (a, b) => {
    const trendingDifference =
      (b.trending_add_count ?? -1) - (a.trending_add_count ?? -1);
