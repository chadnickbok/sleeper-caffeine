...
export function cacheMetadata(
  ...
  if (directory.stale) {
    const ageSeconds = Math.max(
      0,
      Math.floor((Date.now() - Date.parse(directory.fetchedAt)) / 1_000),
    );
    warnings.push({
...
export function rosterView(
  ...
  const join = (playerId: string): PlayerSummary => {
    ...
    if (
      !warnings.some((warning) => warning.code === `UNKNOWN_PLAYER_${playerId}`)
    ) {
      warnings.push({
  ...
  return {
    ...
    starters: starterIds.map((playerId, index) => {
      ...
      return starterSlot === undefined
        ? player
        : { ...player, starter_slot: starterSlot };
    }),
    bench: allIds
      .filter((playerId) => !excludedFromBench.has(playerId))
      .map(join),
...
export function pickInventory(rosterId: number, picks: TradedPick[]) {
  return {
    acquired: picks.filter(
      (pick) => pick.owner_id === rosterId && pick.roster_id !== rosterId,
    ),
    sent: picks.filter(
      (pick) => pick.roster_id === rosterId && pick.owner_id !== rosterId,
    ),
    all_traded_picks_currently_owned: picks.filter(
      (pick) => pick.owner_id === rosterId,
    ),
    scope:
      "Sleeper's traded-picks ledger; untraded native picks are not enumerated",
  };
}

export function userForRoster(
  roster: Roster,
  users: SleeperUser[],
): SleeperUser | undefined {
