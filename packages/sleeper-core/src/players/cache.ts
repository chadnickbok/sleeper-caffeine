...
export class PlayerCache {
  ...
  #refreshSingleFlight(
    staleCache: PlayerCacheFile | undefined,
  ): Promise<PlayerCacheSnapshot> {
  ...
  async #readDisk(): Promise<PlayerCacheFile | undefined> {
    try {
      ...
      return parsed.success && Object.keys(parsed.data.players).length > 0
        ? parsed.data
        : undefined;
    } catch (error) {
  ...
  #isStale(cache: PlayerCacheFile): boolean {
    return (
      this.#now().getTime() - Date.parse(cache.fetchedAt) >= this.#maxAgeMs
    );
  }
