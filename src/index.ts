...
import { PlayerCache } from "./players/cache.js";
import { PlayerDirectory } from "./players/directory.js";
import { SleeperApi } from "./sleeper/endpoints.js";

async function main(): Promise<void> {
  const config = loadConfig();
  const api = new SleeperApi();
  const players = new PlayerDirectory(new PlayerCache(api, { cacheDir: config.cacheDir }));
