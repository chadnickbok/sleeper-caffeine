...
import type { BracketMatch, League, Roster, SleeperUser } from "../sleeper/types.js";
...
export async function getLeagueHistory(
  ...
  while (leagueId !== null && seasons.length < input.max_seasons) {
    if (visited.has(leagueId)) {
      warnings.push({ code: "LEAGUE_HISTORY_CYCLE", message: `League history repeats league ID ${leagueId}.` });
      break;
    }
    ...
    try {
      ...
      const normalized = error instanceof SleeperMcpError ? error.code : "SLEEPER_UNAVAILABLE";
      warnings.push({
...
function findChampion(bracket: BracketMatch[]): number | null {
  const titleMatch = bracket.find((match) => match.p === 1 && match.w !== null && match.w !== undefined);
  if (titleMatch?.w !== undefined && titleMatch.w !== null) {
    return titleMatch.w;
  }
  const completed = bracket.filter((match) => match.w !== null && match.w !== undefined);
  const lastRound = completed.reduce<BracketMatch | undefined>(
    (latest, match) => (latest === undefined || match.r > latest.r ? match : latest),
    undefined,
  );
  return lastRound?.w ?? null;
}

function summarizeHistoricalRoster(roster: Roster | undefined, users: SleeperUser[]) {
  if (roster === undefined) {
    return null;
  }
