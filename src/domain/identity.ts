...
export async function resolveTeam(
  ...
  const candidates = rosters.filter(
    (roster) => roster.owner_id === user.user_id || roster.co_owners?.includes(user.user_id) === true,
  );
  ...
  if (roster === undefined) {
    throw new SleeperMcpError("TEAM_NOT_FOUND", "The resolved Sleeper roster was unavailable.");
  }
  const leagueUser = users.find((candidate) => candidate.user_id === user.user_id) ?? user;
