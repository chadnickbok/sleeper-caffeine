...
export class PlayerDirectory {
  ...
  async get(): Promise<PlayerDirectorySnapshot> {
    ...
    if (this.#index === undefined || this.#indexedFetchedAt !== cache.fetchedAt) {
  ...
  async refresh(): Promise<PlayerDirectorySnapshot> {
    ...
    this.#index = this.#buildIndex(cache.players);
    this.#indexedFetchedAt = cache.fetchedAt;
    return { fetchedAt: cache.fetchedAt, stale: cache.stale, players: this.#index };
  }

  #buildIndex(players: Record<string, SleeperPlayer>): ReadonlyMap<string, PlayerSummary> {
    return new Map(Object.entries(players).map(([id, player]) => [id, toPlayerSummary(id, player)]));
  }
}

export function toPlayerSummary(playerId: string, player: SleeperPlayer): PlayerSummary {
  const derivedName = [player.first_name, player.last_name].filter(Boolean).join(" ").trim();
  return {
    player_id: playerId,
