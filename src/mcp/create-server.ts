...
import { getAvailablePlayers } from "../domain/available-players.js";
import {
...
import { getLeagueHistory } from "../domain/league-history.js";
import { getMatchupContext } from "../domain/matchup-context.js";
import { getTeamSnapshot } from "../domain/team-snapshot.js";
import { getTradeContext } from "../domain/trade-context.js";
import type { DomainDependencies } from "../domain/types.js";
  ...
  server.registerTool(
    ...
    (input) =>
      runTool(
        ...
        (data) => `Found ${String(data["total_matching"])} roster-available players before the result limit.`,
      ),
  );
  ...
  server.registerTool(
    ...
    (input) =>
      runTool(
        ...
        (data) => `Loaded ${String(Array.isArray(data["seasons"]) ? data["seasons"].length : 0)} league seasons.`,
      ),
  );
