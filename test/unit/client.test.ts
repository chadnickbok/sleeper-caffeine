...
describe("SleeperClient", () => {
  it("retries transient responses and validates the result", async () => {
    ...
    const client = new SleeperClient({ fetch, maxRetries: 1, random: () => 0, sleep });
  ...
  it("maps 404 responses to a stable error", async () => {
    const client = new SleeperClient({ fetch: async () => jsonResponse({}, 404), maxRetries: 0 });
  ...
  it("rejects unexpected successful payloads", async () => {
    const client = new SleeperClient({ fetch: async () => jsonResponse({ nope: true }), maxRetries: 0 });

    await expect(client.get("/shape", z.object({ ok: z.boolean() }))).rejects.toMatchObject({
  ...
  it("rejects paths that could replace the fixed host", async () => {
    ...
    await expect(client.get("//example.com", z.object({}))).rejects.toMatchObject({ code: "INVALID_INPUT" });
  });
});
