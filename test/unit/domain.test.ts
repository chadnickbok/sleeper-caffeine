...
import { getAvailablePlayers } from "../../src/domain/available-players.js";
import { getLeagueHistory } from "../../src/domain/league-history.js";
import { getMatchupContext } from "../../src/domain/matchup-context.js";
import { getTeamSnapshot } from "../../src/domain/team-snapshot.js";
import { getTradeContext } from "../../src/domain/trade-context.js";
...
describe("Sleeper domain services", () => {
  it("joins a team snapshot and finds its opponent", async () => {
    ...
    const team = result.data["team"] as { starters: Array<{ name: string }>; bench: Array<{ name: string }> };
    const matchup = result.data["matchup"] as { opponent: { roster_id: number } };
  ...
  it("derives and ranks roster availability without including inactive players", async () => {
    const result = await getAvailablePlayers(await createFixtureDependencies(), {
      league_id: "12345",
    ...
    expect(players.map((player) => player.player_id)).toEqual(["p4", "p6", "SEA"]);
    expect(players[0]).toMatchObject({ roster_availability: true, trending_add_count: 42 });
  ...
  it("returns factual matchup context", async () => {
    ...
    expect((result.data["opponent"] as { roster_id: number }).roster_id).toBe(2);
